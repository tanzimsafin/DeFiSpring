...
contract Presale_Contract is ERC20 {
    ...
    modifier Initial_fundRaised_Only() {
        require(
            presell_fund_raised / (10 ** decimals()) <
                target_fundRaised_ammount,
            "Sorry Initial Fund Raises is Closed"
        );
        _;
    }
    modifier Initial_fundRaised_Completed() {
        require(
            presell_fund_raised / (10 ** decimals()) >=
                target_fundRaised_ammount,
            "sorry still token is not publicly launched "
        );
        _;
    }
    modifier OwnerOnly() {
    ...
    function initial_contribute() public payable Initial_fundRaised_Only {
    ...
    function mintToken(uint _ammount) public OwnerOnly {
        require(
            total_minted_token + _ammount <= 10000000000,
            "Sorry can not mint more tokens"
        );
        _mint(address(this), _ammount);
        total_minted_token += _ammount;
    }
    function Claim_Token() public Initial_fundRaised_Completed {
        require(
            ...
            "sorry you did not buy any token"
        );
        require(total_minted_token > 0, "Sorry still tokens are not minted");
        _transfer(address(this), msg.sender, 100);
        total_minted_token -= 100;
        initial_contributor[msg.sender] = 0;
        afterLaunc_token_buyers[msg.sender] = 0;
    }
    function totalMintCoin() public view returns (uint256) {
        return total_minted_token;
    }
    function initialETHCollection() public view returns (uint256) {
        return (presell_fund_raised) / (10 ** decimals());
    }
    function After_initialETHCollection() public view returns (uint256) {
        return (regular_sell) / (10 ** decimals());
    }
    function totalETHCollection() public view returns (uint256) {
        return (presell_fund_raised + regular_sell) / (10 ** decimals());
    }

    function withDrawMoney() public OwnerOnly {
        uint256 contractBalance = address(this).balance;
        require(
            contractBalance > 0 && contractBalance >= target_fundRaised_ammount,
            "No Ether available to withdraw"
        );
